      ...
      WorldModel world, ImageStore imageStore)
   {
      ...
      case MINER_NOT_FULL:
         scheduleEvent(scheduler, entity,
            ...
            entity.actionPeriod);
         scheduleEvent(scheduler, entity,
            Action.createAnimationAction(entity, 0), getAnimationPeriod(entity));
         break;
      case ORE:
         scheduleEvent(scheduler, entity,
            Action.createActivityAction(entity, world, imageStore),
...
/* //SPECIAL BLACKED OUT CUZ OF ANNOYING COMMENTS
   ...
   public static void drawBackground(WorldView view)
   {
      for (int row = 0; row < view.viewport.numRows; row++)
      {
         for (int col = 0; col < view.viewport.numCols; col++)
         {
            ...
            Optional<PImage> image = WorldModel.getBackgroundImage(view.world,
               worldPoint);
            if (image.isPresent())
            {
